...
// --- NEW: Fix for Back Button Sync (BFCache) ---
// This ensures that when you hit "Back" from the Product Page, the Main Page updates instantly.
...
function filterStatus(status, btnElement) {
    ...
    for (let card of cards) {
        if (status === 'all') {
            ...
            // Check if the card has the specific class ('available' or 'sold')
...
// --- NEW: SPA NAVIGATION FUNCTION (Glides between pages) ---
function showPage(pageId) {
    ...
    // 2. Show the selected page with animation
    const selectedPage = document.getElementById(pageId + '-view');
    if (selectedPage) {
    ...
    navLinks.forEach(link => {
        ...
        link.style.color = "#bbb"; // Reset color
    ...
    if (activeLink) {
        ...
        activeLink.style.color = "#f1c40f";
        activeLink.style.borderBottom = "2px solid #f1c40f";
    }
}